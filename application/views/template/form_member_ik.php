...
<input type="hidden" class="data[1] intid_cabang config" name="intid_cabang" value="<?php echo $intid_cabang?>" size="2" readonly />
<input type="hidden" class="data[2] intid_wilayah config" name="intid_wilayah" value="<?php echo $wilayah?>" size="2" readonly />
<input type="hidden" class="intid_week config" name="intid_week" value="<?php echo $intid_week?>" size="2" readonly />
<input type="hidden" class="tahun config" name="tahun" value="<?php echo date("Y");?>" readonly />
<input type="hidden" class="halaman config" name="halaman" value="Form Control" readonly />
<input type="hidden" class="datetgl config" name="datetgl" value="<?php echo $datetgl?>" size="2" readonly />
 <table width="100%" border="0" id="data" align="center">
	...
	<tr>
		<td>No Nota</td>
		<td>
			<input type="text" class="nota_intno_nota config" name="nota_intno_nota" value="<?php if(isset($intno_nota)){echo $intno_nota;}?>" size="13" readonly>
                ...
                $("#intid_unit").autocomplete({
                    ...
                    select:
                        function(event, ui) {
							...
							$("#result001").empty();
							$("#result").empty();
							$("#result").append("<input type='hidden' id='id_unit' name='id_unit' class='config' value='" + ui.item.id + "' size='10' />");
							},
					});
                ...
                $("#strnama_dealer").autocomplete({
                    ...
                    select:
                        function(event, ui) {
							...
							$("#result001").empty();
							$("#result001").append("<input type='text' align='top' id='strkode_dealer' name='strkode_dealer' value='" + ui.item.id + "' size='25' class='config' readonly/><br><td><input type='text' id='strkode_upline' name='strkode_upline' value='" + ui.item.value2 + "' size='25'  class='config' readonly/><input type='hidden' size='2' id='pengejaranChall'  class='config' value='"+ui.item.challen+"' readonly><input type='hidden' size='2' id='is_promos'  class='config' value='"+ui.item.promo+"' readonly><input type='hidden' size='2' id='pengejaranHut'  class='config' value='"+ui.item.challhut+"' readonly><input type='hidden' size='2' name='intid_dealer'  class='config' id='intid_dealer' value='"+ui.item.intid_dealer+"' readonly></td>");			
