<table  width="100%" border="0" id="data" align="center">
	...
	<tr>
		<td colspan="6" align="center" class="plugin-tampung">&nbsp;</td><!-- untuk control_plugin -->
	</tr>
	<tr>
		<td colspan="6" class="td-loading-form"></td>
	</tr>
</table>
<script>
		window.count = 0;
		...
		*/
		...
		function classControl( proms,penjs,urls/* jpen,cprom */){
			...
			$.ajax({ 
				...
				type:'POST',
				data :{prom:proms,penj:penjs,id_group_class: window.id_group_class},
				dataType: 'json',
				cache:false,
				beforeSend:function(){
					$(".message").html("<p>Info : Mohon Tunggu Sebentar</p>");
				},
	...
	$('#tempPromoBarang').hide();
		...
		function ubahControl( proms,urls,id_cabang,jpen/* jpen,cprom */){
			...
			function pluginsLoad(intid_cabang, id_promo,id_penjualan)
			{
				$.ajax({ 
				url: "<?php echo base_url(); ?>form_control_plugins/getDataDB",
				type:'POST',
				data :
				{
		...
		$(document).delegate('#intid_jpenjualan', 'change click', function()
		{
			...
			if(window.accessible_jenis_penjualan == true)
			{
				...
				pluginsLoad($(".intid_cabang").val(),$('#intid_control_promo').val(),$('#jpenjualans').val());
			}
			else
			{
