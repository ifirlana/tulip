...
<script type="text/javascript">
                $(".id1").autocomplete({
                    ...
                    source:
                        function(req, add){
                        $.ajax({
                            url: "<?php echo base_url(); ?>penjualan/lookupBarangNoMetal",
                            dataType: 'json',
                            type: 'POST',
                            data: req,
							success:
                    ...
                    select:
                        function(event, ui) {
                        $("#result1").html(
                        "<input type='text' id='harga_barang' name='harga_barang' value='" + ui.item.value1 + "' size='15' readonly='readonly'/><input type='hidden' id='id_barang' name='id_barang' value='" + ui.item.id + "' size='15' /><input type='hidden' id='id_pv' name='id_pv' value='" + ui.item.value3 + "' size='15' />"
                    );
							...
							<input type="hidden" id="totallg001" name="totallg001" value="0" />
                               ...
                               <tr>
                                ...
                                <tr>
                                  ...
                                  <td>&nbsp;: Rp.</td>
                                    <td><input type="text" name="intsisa" id="intsisa" />
...
<script type="text/javascript">
    ...
    $('#addBrg').bind('click', function(e){
        ...
        if(($(".id1").val()=="")){
            ...
            var out = '';
            out += 'Banyaknya<br>'; 
			out += '<input id="'+idx+'" name="intquantity[]" type="text" size="2" value="'+jumlah+'" onkeyUp="kali(this.id)"  />&nbsp;';
            out += '<input id="barang001_'+idx+'" name="barang['+idx+'][intid_barang]" type="text" size="50" value="'+brg+'"  />';
            out += '&nbsp;Harga:&nbsp;<input id="harga_'+idx+'" name="intharganew[]" type="text" size="8" value="'+harga+'">';
            ...
            out += '<input name="idPromo[]" id="idPromo_'+idx+'" type="hidden" value="88">';
	...
	function kali(id){
		...
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) 
		{
			...
			if(!isNaN(qty) && !isNaN(hargasatuan))
			{
                ...
                _temppv = _temppv + parseFloat(qty * $("#intpv_"+i).val());
                omset20 = parseInt(omset20) + parseInt($("#total_"+i).val());
				console.log("omset20 "+omset20);
			}
		}	
		//console.log("omset20 "+omset20+", intvoucher "+intvoucher);
		omset20		= omset20 - intvoucher;
        _temppv     = parseFloat(_temppv) - parseFloat($("#temp_pv").val());
		...
		*/
        ...
        $("#voucher").val(parseInt(intvoucher));
        $("#voucher_").val(parseInt(intvoucher));
