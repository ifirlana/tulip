  ...
  $('#frmjual').submit(function(event){
	...
	$('#addBrg').bind('click', function(e){
       if(isNaN($("#harga_barang").val())){
        ...
        }else if(!isNaN($('#harga_barang').val()) && $('#harga_barang').val()!= 0 ){
			...
			/* percobaan */
			out = '<div id="style_'+idx+'" class="pilihPromo1Free1">Promo, Banyaknya<br />';
			out += 'jumlah <input type="text" name="barang['+idx+'][intquantity]" id="'+idx+'" class="main_'+idx+'" min="0" style="width: 50;" size="1" value="'+jumlah+'" onkeyUp="operasiHitung(this.id)" onkeypress="return isNumberKey(event)" />, ';
			...
			/* percobaan */
			out = '<div class="pilihPromo1Free1_free"><sup>(Free)</sup>Banyaknya<br>';
			out += 'jumlah <input type="text" name="barang['+idx+'][intquantity]" id="'+idx+'" class="main_'+idx+'" min="0" style="width: 50;" size="1" value="'+jumlah+'" onkeyUp="operasiFree(this.id)" onkeypress="return isNumberKey(event)" />, ';
...
function counting_1free1(){
	var _temphitung = 0;
	for (var i = 1; i <= parseInt($('#tracker001').val()); i++){	
			if(!isNaN($('#total_'+i).val())){
				_temphitung += parseInt($('#total_'+i).val());
			}
		}
		if(isNaN(_temphitung)){
		_temphitung = 0;
		}
	$('#intjumlahOmset').val(formatAsRupiah(_temphitung));
	$('#intjumlahOmsethidden').val(_temphitung);
	$('#intjumlah1').val();
	$('#intjumlah2').val();
	$('#intjumlahhidden').val(_temphitung);
	$('#intjumlah1hidden').val();
	$('#intjumlah2hidden').val();
	$('#intjumlahtradehidden').val();
	$('#intjumlahfree').val();
	$('#intdebit').val(0);
	$('#intdebit').attr('readonly',true);
	$('#intkkredit').val(0);
	$('#intkkredit').attr('readonly',true);
	$('#intvoucher').val();
	$('#jumlahsementara').val();
	$('#intkomisiasi').val();
	$('#totalbayar').val(formatAsRupiah(_temphitung));
	$('#totalbayarhidden').val(_temphitung);
}
function sisa() {
		var _cash = 0;
		var _kredit = 0;
