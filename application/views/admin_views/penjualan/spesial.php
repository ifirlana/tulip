    ...
    $('#addBrg').bind('click', function(e){
		...
		if(($(".id1").val()=="") &&($(".frees").val()=="")){
		...
		else if(($('.id1').val() != "") && ($('#harga_barang').val() != ""))
			{
				...
				if($('#intid_jpenjualan').attr('value') == 24){
						harga =  Math.ceil(harga * 0.5);
						//alert('cilukba '+harga);
					}
				if ($('#intid_jpenjualan').attr('value')== 11 || $('#intid_jpenjualan').attr('value')== 12|| $('#intid_jpenjualan').attr('value')== 24) 
				{
...
|______________________________________________________________________*/
	...
	*/
	$('#intid_jpenjualan').change(function() {
		...
		{
			var title = "Nota Penjualan Romance";
			$("#title").text(title);
            $("#formpilihbarang").removeAttr('style','style');
            autoCompPromoNormal();
		}
            ...
            for(var i=1; i<= parseInt(id);i++){
				...
				if(jumlah >=0){
					...
					} else if ($("#komisi_"+ i).val()== 20)
					{
						...
						$('#komisi2').val(formatAsRupiah(0));
						$('#komisi2hidden').val(0);
						$('#intjumlah2').val(formatAsRupiah(0));
						$('#intjumlah2hidden').val(0);
						$('#intjumlah1').val(formatAsRupiah(0));
						$('#intjumlah1hidden').val(0);
						$('#komisi1').val(formatAsRupiah(0));
						$('#komisi1hidden').val(0);
					}
