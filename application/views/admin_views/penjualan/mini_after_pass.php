		...
		<div>
				...
				<tr>
					...
					*	2 free 1
                    ...
                    <td><input type="checkbox" name="chkBox10" id="chkBox10" />&nbspVoucher : <input type="text" name="txtpromo10" id="txtpromo10" size="1" /></td>
										else
										*/
										?>
					<!-- @param tulip30komisi10 -->
					<td colspan="3">Tulip 30% komisi 10%</td>
					<td>&nbsp;<input type="checkbox" name="chktulip30_10" id="chktulip30_10" />
						Voucher <input type="checkbox" name="chkvocherbaru" id="chkvocherbaru">
					<td>&nbsp;</td>
                                    	<td>&nbsp;</td>
					<td colspan="3"><!-- Voucher--></td>
					<td>&nbsp;<!-- <input type="checkbox" name="chkV" id="chkV"/> jumlah <input type='text' id="chkV_nominal" size='1' maxlength='1' value='1' onkeypress="return isNumberKey(event)" onkeyup="kali();" >
                                    	--></td>
                                </tr>
					<tr>
					<td>&nbsp;</td>
					...
					*	2 free 1
                    ...
                    <td><input type="checkbox" name="chkBox10" id="chkBox10" />&nbspVoucher : <input type="text" name="txtpromo10" id="txtpromo10" size="1" /></td>
										...
										*/
										?>
					<!-- @param tulip30komisi10 -->
					<td colspan="3">Tulip Komisi 20% Voucher</td>
					<td>&nbsp;<input type="checkbox" name="chkV" id="chkV"/> jumlah <input type='text' id="chkV_nominal" size='1' maxlength='1' value='1' onkeypress="return isNumberKey(event)" onkeyup="kali();" >
                                    	</td>
                                </tr>
...
<script type="text/javascript">
	...
	$('#addBrg').bind('click', function(e){
		if($(".id1").val()==""){
			...
			else if($("#chkBox10").attr('checked') == true || $("#chktulip30_10").attr("checked") == true){ // tulip30komisi10 
			...
			// tulip30komisi10
			 if($("#chktulip30_10").attr("checked") == true && $("#chkvocherbaru").attr("checked") == true){
				...
				console.log('tulip 30 hitung + voucher');
				// harga = harga * 0.7;
			...
			else if($("#chktulip30_10").attr("checked") == true){ // tulip30komisi10 
			...
			out += '<input id="modifier_'+idx+'" name="barang['+idx+'][promoada]" type="hidden" size="5" value="'+modifier+'" readonly>';
					...
					if ($("#chkBox10").attr('checked') == true /*&& _statepromo10 == 1*/)
					{
...
|______________________________________________________________________*/
	...
	*/
	function autoComp() {
		if ($("#chkBox20").attr('checked') == true) {
			...
			if($("#chktulip30_10").attr("checked") == true){ // tulip30komisi10 
	...
	function autoCompPromo20Bayar() {
		$(".id1").autocomplete({
                    ...
                    source:
                        function(req, add){
                        $.ajax({
                            url: "<?php echo base_url(); ?>penjualan/lookupBarangPromoMini",
                            dataType: 'json',
                            type: 'POST',
                            data: {
                                term: req.term,
	...
	*/
	...
	$("#txtpromo10").keyup(function(){
		...
		{
			alert("Anda belum memasukan nama barang / barang tidak sesuai");
			$("#txtpromo10").val(0);
		}
		else
		{
	...
	*/
	function kali(id){
		...
		if ($('#tracker004').val() > _totalQuantityFree) { // tulip30komisi10
...
|______________________________________________________________________*/
	...
	* diskon30%
	*/
	function autoCompPromoDis30() {
		$(".id1").autocomplete({
                    minLength: 5,
                    source:
                        function(req, add){ 
                        $.ajax({
                            url: "<?php echo base_url(); ?>penjualan/lookupBarangMini2030",
                            dataType: 'json',
                            type: 'POST',
                            data: req,
							success:
