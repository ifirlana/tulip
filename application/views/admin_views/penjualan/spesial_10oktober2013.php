    ...
    $('#addBrg').bind('click', function(e){
		...
		if(($(".id1").val()=="") &&($(".frees").val()=="")){
			...
			if(($('.frees').val() != "") && ($('#harga_barang_free').val() != ""))
			{
				...
				out += '<input id="'+idx+'" class="semua_free_'+idx+'" name="barang_free['+idx+'][intquantity]" type="text" size="1" value="'+jumlah+'" onkeyUp="kali(this.id)" onkeypress="return isNumberKey(event)" />&nbsp;';
...
|______________________________________________________________________*/
    ...
    function kali(id){
		...
		//jika kondisi selain jenis penjualan reguler maka harus berjumlah 1
		if($('#intid_jpenjualan').val() != 1){
			if($('#'+id).val() != 1){
