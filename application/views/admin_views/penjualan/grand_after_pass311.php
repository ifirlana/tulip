		...
		<div>
						...
						<input type="hidden" name="halaman" readonly="readonly" value="grand">
                            ...
                            <table width="685" border="0" id="data" align="center">
                                ...
                                <tr>
                                    ...
                                    <td><input type="checkbox" name="chkBox20" id="chkBox20" />
                                        </td>
					<td>&nbsp;</td>
                                    	<td>&nbsp;</td>
                              </tr>
				<tr>
					...
					<td colspan="3">Paket Promo 10%</td>
                                    	<td><input type="checkbox" name="chkBox10" id="chkBox10" />&nbspVoucher : <input type="text" name="txtpromo10" id="txtpromo10" size="1" />
                                        </td>
					<td>&nbsp;</td>
                                    	<td>&nbsp;</td>
				</tr>
                                <tr style="display:none;">
									...
									<td><input type="checkbox" name="chkV" id="chkV"/> jumlah <input type='text' id="chkV_nominal" size='1' maxlength='1' value='0' onkeypress="return isNumberKey(event)" onkeyup="kali();" >
														</td>
                                </tr>
					...
					<td>&nbsp;</td>
                                    	<td>&nbsp;</td>
					<td colspan="3">Harga 50%</td>
					<td><input type="checkbox" name="chkharga50" id="chkharga50" />
                                    	</td>
                                </tr>
								<?}?>
								<tr id="showattrlainlain5"  >
									...
									<td >
										<input type="checkbox" name="chktulip30_10" id="chktulip30_10" /> Voucher:<input type="checkbox" name="chkvocherbaru" id="chkvocherbaru" />
                                    </td>
                                </tr>
								<tr style="display:true;">
									<td>&nbsp;</td>
														<td>&nbsp;</td>
									<td colspan="3">Voucher 25000</td>
									<td><input type="checkbox" name="chktulipbetobe" id="chktulipbetobe" />
									</td>
                                </tr>
								...
								<tr style="display:true;">
									<td>&nbsp;</td>
														<td>&nbsp;</td>
									<td colspan="3">Voucher </td>
									<td><input type="checkbox" name="chkVoEfan" id="chkVoEfan" />
									</td>
                                </tr>
...
<script type="text/javascript">
	...
	$('#addBrg').bind('click', function(e){
		if($(".id1").val()==""){
			...
			out += 'Banyaknya<br>';
			//untuk voucher 20000 be to be
			 if($("#chktulipbetobe").attr("checked") == true){
				var _vocherbaru =25000;
				var _pvvocher = 0.25;
				console.log('tulip voucher 25000 hitung');
				// harga = harga * 0.7;
				harga =  Math.ceil((harga - _vocherbaru) );
				pv	=	(parseFloat(pv - _pvvocher)).toFixed(2);
				autoComp();
				kali();
			 }
			 if($("#chkVoEfan").attr("checked") == true ){
				var _vocherbaru = 0;
				var _pvvocher = 0;
			...
			//untuk tulip 30 10
			 if($("#chktulip30_10").attr("checked") == true && $("#chkvocherbaru").attr("checked") == true){
				var _vocherbaru = 0;
				var _pvvocher = 0;
				promoada = 0.7;
				if ($('#id_wilayah').val() == 1){
				...
				console.log('tulip 30 hitung');
				// harga = harga * 0.7;
				harga =  Math.ceil((harga - _vocherbaru) * 0.7);
				if(harga <= 0){
				harga = 0;
				}
				pv	=	(parseFloat(pv - _pvvocher) * 0.7).toFixed(2);
				if(pv <= 0){
				pv = 0;
				}
				autoComp();
			...
			else if($("#chkBox10").attr('checked') == true || $("#chktulip30_10").attr("checked") == true){
			...
			if($("#chktulipbetobe").attr("checked") == true || $("#chkVoEfan").attr("checked") == true || $("#chkvocherbaru").attr("checked") == true)
			{
				out += '<input id="voucherada'+idx+'" name="barang['+idx+'][voucherada]" type="hidden" size="5" value="'+voucherada+'" readonly> Total Voucher <input type="text" class="voucher_total'+idx+'" value="'+voucherada+'" size="4" readonly>';
			}
			else
			{
				out += '<input id="voucherada'+idx+'" name="barang['+idx+'][voucherada]" type="hidden" size="5" value="'+voucherada+'" readonly><input type="hidden" class="voucher_total'+idx+'" value="'+voucherada+'" size="4" readonly>';
			}
					//001
					...
					out = '<div style="height:60px">' + out + '</div><br /><br />';
			$(out).insertBefore('#ButtonAdd');
			
				$("#chktulipbetobe").attr("checked",false);
				$("#chkVoEfan").attr("checked",false);
				$("#chkvocherbaru").attr("checked",false);
			//001
			ajaxgila(idx);
	...
	function kali(id){
		...
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			if($('.semua_'+ i).val() == '') {
				$('#total_' + i).val(0);
				//ikhlas
				$(".voucher_total"+i).val(parseFloat($('.semua_'+ i).val() * $('#voucherada'+ i).val()));
		...
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			if($('.sepuluh_'+ i).val() == '') {
				$('#total_' + i).val(0);
				//ikhlas
				$(".voucher_total"+i).val(parseFloat($('.sepuluh_'+ i).val() * $('#voucherada'+ i).val()));
		...
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			if($('.duapuluh_'+ i).val() == '') {
				$('#total_' + i).val(0);//ikhlas
				$(".voucher_total"+i).val(parseFloat($('.duapuluh_'+ i).val() * $('#voucherada'+ i).val()));
		...
		if($("#chktulip30_10").attr("checked") == false){
