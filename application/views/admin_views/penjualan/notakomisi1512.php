...
////////////////////////////////////////////////////////
    ...
    <div id="page">
        <div id="page-bgtop">
            <div id="content">
                <div>	<h2 class="title">penjualan diskon 20% komisi 15% Voucher</h2>
                    <div class="entry">
                        <form action="<?php echo base_url()?>penjualan/notad20k15" method="post" name="frmjual" id="frmjual">
						<input type="hidden" name="halaman"  value="d20k15">
                            ...
                            <table width="685" border="0" id="data" align="center">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" name="chkBox20" id="chkBox20" disabled="disabled" style="display:none;"/>
                                ...
                                <tr style="display:none;">
                                    ...
                                    <td><input type="checkbox" name="chkBoxTrade" id="chkBoxTrade" disabled="disabled" style="display:none;"/>
							  ...
							  */
                              ...
                              <tr id="showattrlainlain3" style="display:none;">
								...
								<tr id="showattrlainlain3">
									<td>&nbsp;</td>
                                    <td>&nbsp;</td>
									<td colspan="3">Promo 20% komisi 15%</td>
									<td ><input type="checkbox" name="chktulip20_15" id="chktulip20_15"  /> Voucher:<input type="checkbox" name="chkvocherbaru" id="chkvocherbaru" /> 
                                    </td>
                                </tr>
                                <tr>
...
<td width="63" rowspan="3"><div id="data">
                                ...
                                <tr>
                                    ...
                                    <td>Omset 10%</td>
									<td>:</td>
									<td> Rp.<input type="text" name="jml10" id="intjumlah1" readonly="readonly"/>
								...
								<tr>
                                    ...
                                    <td>Omset 15%</td>
									<td>:</td>
									<td> Rp.<input type="text" name="jml15" id="intjumlah15" readonly="readonly"/>
										<input type="hidden" name="jml15" id="intjumlah15hidden"/>
									</td>
								</tr>
								<tr>
                                    <td>&nbsp;</td>
                                    ...
                                    <td>Omset 20%</td>
									<td>:</td>
									<td> Rp.<input type="text" name="jml20" id="intjumlah2" readonly="readonly"/>
                                ...
                                <tr>
                                    ...
                                    <td>:</td>
                                    <td>
                                        Rp.<input type="text" name="komisi15" id="komisi15" readonly="readonly"/>
                                        <input type="hidden" name="komisi15hidden" id="komisi15hidden"/>                                    </td>
                                </tr>
                                <tr>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
...
//001
	...
	if($("#track6").val() == 0) {$("#metal_6").removeAttr("disabled");}
        ...
        if(($(".id1").val()=="") &&($(".frees").val()=="")){
            ...
            else if($("#chktulip20_15").attr('checked') == true ) {
                out += '<input id="'+idx+'" class="limabelas_'+idx+'" name="barang['+idx+'][intquantity]" type="text" size="1" value="'+jumlah+'" onkeyUp="kali(this.id)" onkeypress="return isNumberKey(event)" />&nbsp;';
                out += '<input type="hidden" id="hit15" name="hit15[]" value="'+idx+'">';
            }
			...
			if($("#chktulip50_10").attr("checked") == true){
				...
				if($("#chktulip20_15").attr("checked") == true && $("#chkvocherbaru").attr("checked") == true){
					var _vocherbaru = 0;
					var _pvvocher = 0;
					modifier = 0.8;
					if($('#is_voucher_promo').val() != 0){
					
						if ($('#id_wilayah').val() == 1){
							_vocherbaru = 50000 ;
							_pvvocher = 0.5;
							voucherada = _vocherbaru;
						} else {
						voucherada = _vocherbaru;
							_vocherbaru = 60000 ;
							_pvvocher = 0.6;
					...
					harga = ((harga - _vocherbaru) * 0.8);  
					pv	=	(parseFloat(pv - _pvvocher) * 0.8).toFixed(2);
					autoComp();
					kali();
				}
				else if($("#chktulip20_15").attr("checked") == true){
			...
			out += '<input id="nominal_'+idx+'" name="barang['+idx+'][harga_asli]" type="hidden" size="5" value="'+nominal+'" readonly>';
			out += '<input id="nominal_voucher_'+idx+'" name="barang['+idx+'][voucherada]" type="hidden" size="5" value="'+voucherada+'" readonly>';
			out += '<input id="modifier_'+idx+'" name="barang['+idx+'][promoada]" type="hidden" size="5" value="'+modifier+'" readonly>';
...
//001
	...
	if($('#intid_jpenjualan').val() != 7) {
		...
		/* start komisi 15 */
			...
			var csfree = parseInt($('#hit15').val());
			var tt = limit10 * 2;
			

	    	var ju=0;
            for(var i=parseInt(csfree); i<= parseInt(id);i++){
                var jumlah = parseInt($('.limabelas_'+ i).val());
                var harga = parseInt($('#harga_' + i).val());
		...
		if($('#chkV').attr('checked') == true && total10 < 0)
		{
            ...
            $('#intjumlah15').val(formatAsDollars(total10));
            $('#intjumlah15hidden').val(total10);
            var total = parseInt($('#intjumlah15hidden').val());
	...
	if($('#intid_jpenjualan').val() != 7) {
		...
		if($('#chkV').attr('checked') == true && komisi < 0)
		{
            ...
            if($('#komisi15hidden').val()!=""){
               	var totkom = komisi;
                $('#komisi15hidden').val(totkom);
                $('#komisi15').val(formatAsDollars(totkom));
            }else{
                $('#komisi15hidden').val(komisi);
                $('#komisi15').val(formatAsDollars(komisi));
            }
	...
	if($('#intid_jpenjualan').val() != 7) {
		...
		if($('#komisi15hidden').val() == ""){
            ...
            var kom15 = parseInt($('#komisi15hidden').val());
        }
		...
		if($('#intjumlah15hidden').val() == ""){
            var intjum15 = 0;
        }else{
            var intjum15 = parseInt($('#intjumlah15hidden').val());
        }
        ...
        var omset = intjum1 + intjum15 + intjum2 + intjumfree + intjumt;
...
|______________________________________________________________________*/
	...
	*/
	function autoComp() {
		...
		if($("#chktulip20_15").attr("checked") == true){
			console.log('hut ');
			autoCompPromoD20K15();
		 }else{
		 console.log();
		...
		//	alert("4");
			...
			if($("#chktulip50_10").attr("checked") == true){ //tulip 50 komisi 10
				...
				}else if($("#chktulip20_15").attr("checked") == true){ //tulip 50 komisi 10
				
				autoCompPromoD20K15();//config
				}else{
	...
	*/
	...
	/*
	* auto complete bwt promo diskon 20 komisi 15	
	*/
	function autoCompPromoD20K15() {
		$(".id1").autocomplete({
                    minLength: 5,
                    source:
                        function(req, add){
                        $.ajax({
                            url: "<?php echo base_url(); ?>penjualan/lookupBarangd20k15",
                            dataType: 'json',
                            type: 'POST',
                            data: req,
							success:
			...
			$(this).val() = q;
                    ...
                    select:
                        function(event, ui) {
                        $("#result1").html(
                        "<input type='text' id='harga_barang' name='harga_barang' value='" + ui.item.value1 + "' size='5' readonly='readonly'/><input type='hidden' id='id_barang' name='id_barang' value='" + ui.item.id + "' size='15' /><input type='hidden' id='pv' name='pv' value='" + ui.item.value3 + "' size='15' readonly='readonly'/><input type='hidden' id='id_harga' name='id_harga' value='" + ui.item.value7 + "' size='15' /><input type='hidden' id='is_voucher_promo' name='is_voucher_promo' value='" + ui.item.is_voucher + "' size='15' />"  
                    );
	...
	*/
	function kali_baru(id){
		...
		//itung jumlah barang omset 15 total yg sudah di order dibantu pakai tracker bwt tau brp row barang yg udah keluar
		var _totalQuantity15 = 0;
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			if (parseInt($('.limabelas_'+ i).val()) >= 0) {
				if (parseInt($('.limabelas_'+ i).val()) != '')
				{
					console.log('_totalQuantity15 '+_totalQuantity15);
					_totalQuantity15 += parseInt($('.limabelas_'+ i).val());
				}
			}
		}
		...
		//harga total barang omset 15 setelah harga dikalikan dengan jumlah
		var _totalHarga15 = 0;
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			if($('.limabelas_'+ i).val() == '') {
				$('#total_' + i).val(0);
			} else if (parseInt($('.limabelas_'+ i).val()) >= 0) {
				$('#total_' + i).val(parseInt($('.limabelas_'+ i).val()) * parseInt($('#harga_' + i).val()));
				_totalHarga15 += parseInt($('#total_' + i).val());
			}
		}
		...
		$('#intjumlah15').val(_totalHarga15);
		$('#intjumlah2').val(_totalHarga20 + _totalHargaNormal);
		$('#intjumlah').val(parseInt($('#intjumlah1').val()) + parseInt($('#intjumlah15').val()) + parseInt($('#intjumlah2').val()) + _totalHargaFreeHUT + _totalHargaFrees);
		...
		//hitung komisi 15% dan kurangi totalbayar
		if ($('#intjumlah15').val() != '' && $('#intid_jpenjualan').attr('value') != 4 && $('#intid_jpenjualan').attr('value') != 7 && $('#intid_jpenjualan').attr('value') != 8)
		{
			$('#komisi15').val(parseInt($('#intjumlah15').val()) * 0.15);
			$('#totalbayar').val(parseInt($('#totalbayar').val()) - parseInt($('#komisi15').val()));
		}
		...
		$('#intjumlah15hidden').val($('#intjumlah15').val());
		...
		$('#intjumlah15').val(formatAsRupiah(parseInt($('#intjumlah15').val())));
