...
<div id="page1">
    ...
    <div id="page">
        <div id="page-bgtop">
            <div id="content">
                <div>	<h2 class="title">Combo</h2>
		<div>
                    <div id="entry" class="entry" >
                        <form id="form-action" action="<?php echo base_url()?>penjualan/dp" method="post" name="frmjual" id="frmjual">
						<input type="hidden" name="halaman" readonly="readonly" value="combo">
				...
				<tr>
					...
					<td><input type="hidden" name="chkSmart" id="chkSmart" />
                                ...
                                <tr>
                                    <td colspan="6">&nbsp;
                                    <input type="hidden" id="id_wilayah" value="<?php echo $intid_wilayah;?>" readonly />
                                    
                                        <div align="center" id="title"></div></td>
                                </tr>
                            </table>
                           <?php $this->load->view("penjualan/combo/submenu");?>
						   <!-- Start-->
						<table id="data" style="width:100%;">
							...
							<script>
								...
								*/
								$(document).ready(function(){
									$("#form-action").attr("action","<?php echo base_url()?>penjualan/combo_after");
									...
									$("#DP").bind("click",function(){
										...
										if($("#DP").attr("checked") == true){
											...
											$("#form-action").attr("action","<?php echo base_url()?>penjualan/combo_after");
												...
												$("#intdp").val('');
												$("#form-action").attr("action","<?php echo base_url()."penjualan/dp";?>");
												}
										});
/* 									
	...
	$('#addBrg').bind('click', function(e){
		if($(".id1").val()==""){
			...
			out += '&nbsp;Total:&nbsp;<input id="total_'+idx+'" name="barang['+idx+'][intid_total]" type="text" size="5" value="0" readonly>';
					...
					{
						out += '<br /><input type="checkbox" id="voucher_'+idx+'" onclick="kali()" />Voucher<input type="text" size="1" id="nominal_voucher_'+idx+'" onkeyUp="kali(this.id);" onkeypress="return isNumberKey(event)" value="0" />&nbsp&nbsp';
					}
...
|______________________________________________________________________*/
	...
	*/
	function kali(id){
		...
		console.log("totalquantitycombo = "+_totalQuantitycombo);
        ...
        for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			...
			console.log("total harga combo out :"+_totalHargacombo);
		...
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
			if (parseInt($('#total_'+ i).val()) >= 0 && parseInt($('#harga_'+ i).val()) >= 0 && parseFloat($('#pv_'+ i).val()) >= 0) {
                ...
                else if(parseInt($('.combo_'+ i).val()) > 0){ // total free combo
		...
		//perlu perbaikan ckck
		var _totalv10 = parseInt($('#txtpromo10').val()) * _v;
		if(isNaN(_totalv10)){
			_totalv10 = 0;
		}
		
		console.log("++total = "+_totalv10+", "+_totalHarga10+" "+_totalHargacombo);
		/***/
		//perbaikan
		for (var i = 1; i <= parseInt($('#tracker001').val()); i++) {
		...
		console.log("--totalHargacombo = "+_totalHargacombo);
		...
		console.log("intjumlah1 = "+_totalHarga10+", "+_totalHargacombo);
...
////999
	...
	*/
	function kali_sepuluh(id) {
		...
		if (_temp > _totalQuantityFree) {
			alert("Anda masih boleh memilih barang free :"+_temp);
			$('#tracker002').val("free");
		} else if (_temp == _totalQuantityFree) {
			alert("Silakan pilih promo berikutnya : "+_temp);
			$('#tracker002').val("bayar");
            _statepromo10 = 0;
			load_db_promo_combo();
            //$(".id1").focus();
			...
			alert("Jumlah barang free melebihi quota : "+_temp);
			$('.free_'+ id).val('');
		}
	...
	*/
	function sisa() {
		...
		var _bayar=	0;
		$("input[type=submit]").removeAttr("disabled");
		if ($('#intcash').val() == '' && $('#intkkredit').val() == '' && $('#intdebit').val() == '' && $('#intdp').val() == '')
		{
			$('#intsisa').val('');
			$('#intsisahidden').val('');
			$("input[type=submit]").attr('disabled','disabled');
			
			console.log("=== _bayar "+_bayar+", _cash "+_cash+", _kredit "+_kredit+", _debit "+_debit+", _dp"+_dp);
			...
			if ($('#intdp').val() != '') {_dp = parseInt($('#intdp').val());}
			_bayar = _cash + _kredit + _debit + _dp;
